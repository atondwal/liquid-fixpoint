...
command me !cmd      = {-# SCC "command" #-} say cmd >> hear cmd
  where
    ...
    hear (Interpolate fi p q) = smtRead me >>= \case
      Unsat -> smtPred fi me
...
smtDoInterpolate :: Context -> SInfo a -> Pred -> Pred -> IO Pred
...
smtInterpolate :: Context -> SInfo () -> Pred -> Pred -> IO Pred
